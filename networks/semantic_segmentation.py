...
class SemanticBranch(nn.Module):
    ...
    def forward_once(self, vw_features, coord_ind, full_coord, pw_label, info):
        ...
        input_tensor = spconv.SparseConvTensor(vw_features, coord.int(), np.array(self.sizes, np.int32)[::-1], batch_size)
        ...
        proj1_bev = proj1_bev.unsqueeze(2)  # (B, C, 1, H, W)
        proj1_bev_mamba = self.mamba_layer_3d_proj1(proj1_bev)
        proj1_bev = proj1_bev_mamba.squeeze(2)  # (B, C, H, W)
        ...
        proj2_bev = proj2_bev.unsqueeze(2)  # (B, C, 1, H, W)
        proj2_bev_mamba = self.mamba_layer_3d_proj2(proj2_bev)
        proj2_bev = proj2_bev_mamba.squeeze(2)  # (B, C, H, W)
        ...
        proj3_bev = proj3_bev.unsqueeze(2)  # (B, C, 1, H, W)
        proj3_bev_mamba = self.mamba_layer_3d_proj3(proj3_bev)
        proj3_bev = proj3_bev_mamba.squeeze(2)  # (B, C, H, W)
